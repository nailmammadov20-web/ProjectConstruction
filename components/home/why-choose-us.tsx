  ...
  return (
    <section className="section-padding bg-muted/40">
      <div className="container-wide">
        ...
        <RevealGroup className="mt-10 grid grid-cols-1 gap-5 sm:mt-14 sm:grid-cols-2 lg:grid-cols-3">
          {items.map(({ key, icon: Icon }, index) => (
            // Only the first 3 show on mobile — a 6-card single-column stack
            // made the homepage excessively long on small screens.
            <RevealItem key={key} className={cn(index >= 3 && "hidden sm:block")}>
