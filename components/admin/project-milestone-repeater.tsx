...
import { Plus, Trash2, ChevronUp, ChevronDown, History } from "lucide-react";
import type { ProjectMilestone, Locale } from "@/lib/types";

export function ProjectMilestoneRepeater({
  name,
  label,
  defaultValue,
  hint,
  ...
  name: string;
  label: string;
  defaultValue: ProjectMilestone[];
  hint?: string;
  activeLocale?: Locale;
  onActiveLocaleChange?: (locale: Locale) => void;
}) {
  const [items, setItems] = React.useState<ProjectMilestone[]>(defaultValue.length ? defaultValue : []);
  ...
  function addItem() {
    setItems((prev) => [...prev, { date: { en: "" }, label: { en: "" } }]);
  }
  ...
  function updateItem(index: number, patch: Partial<ProjectMilestone>) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        >
          <Plus className="size-3.5" /> Mərhələ əlavə et
        </Button>
      </div>
      ...
      <div className="mt-3 space-y-3">
        {items.length === 0 && (
          <div className="flex flex-col items-center gap-2 rounded-sm border border-dashed border-border py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground">Hələ mərhələ əlavə edilməyib.</p>
          </div>
        )}
