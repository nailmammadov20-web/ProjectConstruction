...
import type { TimelineEntry, Locale } from "@/lib/types";
...
export function TimelineRepeater({
  ...
  function addItem() {
    setItems((prev) => [...prev, { year: "", title: { en: "" }, body: { en: "" } }]);
  }
  ...
  function updateItem(index: number, patch: Partial<TimelineEntry>) {
      ...
      <div className="mt-3 space-y-3">
        {items.length === 0 && (
          <div className="flex flex-col items-center gap-2 rounded-sm border border-dashed border-border py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground">Hələ xronologiya mərhələsi əlavə edilməyib.</p>
          </div>
        )}
